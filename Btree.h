...
private:
    ...
    bool  Encontrar( B_node<Type, order> *current, NodoFT *&target);
    NodoFT*  EncontrarNodoFT( B_node<Type, order> *current, int target);

public: // publics.
	Btree();
	// travesling
	void inOrder();
    bool buscar( NodoFT *&searchitem );
    NodoFT* buscarNodoFT( int searchitem );
	void mostrarArbol();
	// insertion
    bool buscarEnNodo( B_node<Type, order> *current, NodoFT *&target, int &position );
    NodoFT* buscarNodoFTEnNodo( B_node<Type, order> *current, const int target, int &position );
    void insertar(NodoFT *&new_entry);
    void insertar(NodoFT &new_entry,NodoFT*NodoFT);
    void insertarEnRecursion( B_node<Type, order> *current, NodoFT *&new_entry,NodoFT *&median,B_node<Type, order> * &rightchilds , bool &result );
    void insertarEnNodo(B_node<Type, order> *current, NodoFT *&entry, B_node<Type, order> *rightchilds, int position);
    void dividirNodo( B_node<Type, order> *current,  NodoFT *&extra_entry,   B_node<Type, order> *extra_childs,  int position,
        B_node<Type, order> * &right_half,   NodoFT *&median);
	// eliminacion

    void remover( NodoFT *&target );
    void removerEnRecursion( B_node<Type, order> *current, NodoFT *&target );
...
template <class Type, int order> bool Btree<Type,order>::buscar( NodoFT *&searchitem ){
	
	return Encontrar( root , searchitem );
}
template <class Type, int order> bool Btree<Type,order>::Encontrar( B_node<Type, order> *current, NodoFT *&target ){
...
template <class Type, int order> bool Btree<Type,order>::buscarEnNodo( B_node<Type, order> *current, NodoFT *&target, int &position ){
	position=0;
...
template <class Type, int order> NodoFT* Btree<Type,order>::buscarNodoFT( int searchitem ){

    return EncontrarNodoFT( root , searchitem );
}
template <class Type, int order> NodoFT* Btree<Type,order>::EncontrarNodoFT( B_node<Type, order> *current, int target ){

    NodoFT *result = NULL;
    int position;
    if (current != NULL) {
        result = buscarNodoFTEnNodo(current, target, position);
        if (result == NULL)
            result = EncontrarNodoFT(current->childs[position], target);
        else
...
template <class Type, int order> NodoFT* Btree<Type,order>::buscarNodoFTEnNodo( B_node<Type, order> *current, const int target, int &position ){
    position=0;
...
template <class Type, int order> void Btree<Type,order>::insertar(NodoFT *&new_entry){
    NodoFT *median;
...
template <class Type, int order> void Btree<Type,order>::insertar(NodoFT &new_entry,NodoFT*NodoFT){
    Type median;
    B_node<Type, order> *rightchilds, *new_root;
    bool  result ;
    insertarEnRecursion(root, new_entry, median, rightchilds , result);
    if(result) {
        new_root = new B_node<Type, order>(NodoFT);
        new_root->count = 1;
        new_root->data[0] = median;
        new_root->childs[0] = root;
        new_root->childs[1] = rightchilds;
...
template <class Type, int order> void Btree<Type,order>::insertarEnRecursion( B_node<Type, order> *current,
    NodoFT *&new_entry,NodoFT *&median,B_node<Type, order> * &rightchilds , bool &result ){
...
template <class Type, int order> void Btree<Type,order>::insertarEnNodo(B_node<Type, order> *current,
    NodoFT *&entry, B_node<Type, order> *rightchilds, int position){
		int i; 
...
template <class Type, int order> void Btree<Type,order>::dividirNodo( B_node<Type, order> *current,  NodoFT *&extra_entry,   B_node<Type, order> *extra_childs,  int position,
    B_node<Type, order> * &right_half,   NodoFT *&median){
...
template <class Type, int order> void Btree<Type,order>::remover( NodoFT *&target ){
...
template <class Type, int order> void Btree<Type,order>::removerEnRecursion( B_node<Type, order> *current, NodoFT *&target ){
